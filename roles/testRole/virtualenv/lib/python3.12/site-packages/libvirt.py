...
class virDomain(object):
    ...
    def graphicsReload(self, type, flags=0):
        """Reload domain's graphics. This can be used to reload TLS certificates
        without restarting the domain. """
        ret = libvirtmod.virDomainGraphicsReload(self._o, type, flags)
        if ret == -1:
            raise libvirtError('virDomainGraphicsReload() failed')
        return ret
...
class virNodeDevice(object):
    ...
    def update(self, xmlDesc, flags=0):
        ...
        ret = libvirtmod.virNodeDeviceUpdate(self._o, xmlDesc, flags)
        if ret == -1:
            raise libvirtError('virNodeDeviceUpdate() failed')
        return ret
...
VIR_DOMAIN_LAUNCH_SECURITY_SEV_SNP_POLICY = "sev-snp-policy"
...
VIR_MIGRATE_PARAM_MIGRATE_DISKS_DETECT_ZEROES = "migrate_disks_detect_zeroes"
